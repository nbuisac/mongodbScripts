...
# Comprovem les variables d'entorn
mongoDbConnection = os.environ.get("MongoDBConnection", "localhost").replace("\"", "")

client = pymongo.MongoClient(mongoDbConnection)
db = client.school
print(db.name)

print("Connectant a ", mongoDbConnection, "...")
try:
    ## Crea o es connecta a una base de dades
    estudiants = db.students
    mostraTots(estudiants)
except:
    print("Impossible connectar a", mongoDbConnection)
    exit(1)
else:
    print("Connectat a ", mongoDbConnection)
